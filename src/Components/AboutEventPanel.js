...
function AboutEventPanel(props) {
    ...
    React.useEffect(() => {
        ...
        if (event._id !== undefined && event.user_id !== undefined) {
            fetch(process.env.REACT_APP_API_URL + `/api/admin/event/get_organizer_details?eventId=${event._id}&userId=${event.user_id}`, {
                headers: {
            ...
            {value === index && (
                <div className={classes.root}>
                    <Grid container component="main">
                        ...
                        {/* </Grid> */}
                        <Grid item xs={12}>
                        {/* {event !== null &&<Typography>{props.event}</Typography>} */}
                        </Grid>
                        <Grid>
                        </Grid>
                    </Grid>
